...
-- -----------------------------------------------------
-- Table `aula07`.`tipo_cliente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `aula07`.`tipo_cliente` (
  `i_tipocliente_tipocliente` INT NOT NULL,
  `s_desctipocliente_tipocliente` VARCHAR(100) NULL,
...
CREATE TABLE IF NOT EXISTS `aula07`.`cliente` (
  ...
  INDEX `fk_cliente_tipo_cliente_idx` (`i_tipo_cliente` ASC) VISIBLE,
  CONSTRAINT `fk_cliente_tipo_cliente`
    FOREIGN KEY (`i_tipo_cliente`)
    REFERENCES `aula07`.`tipo_cliente` (`i_tipocliente_tipocliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
